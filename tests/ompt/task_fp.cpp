...
#pragma omp taskwait
    ...
    for(auto it = task_type.begin(); it !=task_type.end(); ++it){
        if(it->first!=1 && it->first != 4) return 1;
    }
    if(task_type[1] != 1 || task_type[4] != 5) return 1;
    ...
    for(auto it = return_address.begin(); it != return_address.end(); ++it){
        if(it->second != 1 && it->second != 4) return 1;
    }
    ...
    for(auto it = count_task_schedule_id.begin(); it != count_task_schedule_id.end();++it){
        if(it->second != 0) return 1;
    }
